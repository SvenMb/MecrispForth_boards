\ ---
\ subject: unicode fixed font 8x8 size
\ origin: mecrisp2.5.8 distribution (common/graphics/graphics-unicode-8x8.txt)
\ author: Sven Muehlberg
\ copyright: see mecrisp from Mathias Koch
\ ---

( fixed8x8 start: ) here dup hex.
...
: fixed8x8 ( x -- c-addr width pixels) \ Translates Unicode character to address of 8x8 bitmap.
    ...
    8 64 \ add size of char, 8 pix wide, 64 pix per char 
...
\ ['] fixed8x8 font-hook !
\ : helloworld s" Hello World!" 0 8 drawstring s" (äöüß$€¤£ °¶)" 0 16 drawstring ;

( fixed8x8 end: ) here dup hex.
( fixed8x8 size: ) swap - hex.
